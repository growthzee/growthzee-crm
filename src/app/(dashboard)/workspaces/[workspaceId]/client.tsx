...
export const TaskList = ({ data, total }: TaskListProps) => {
  ...
  const workspaceId = useWorkspaceId();
  return (
    <div className="flexx flex-col  gap-y-4 col-span-1">
          ...
          {data.map((task) => (
            <li key={task.$id}>
              <Link href={`/workspaces/${workspaceId}/tasks/${task.$id}`}>
                <Card className="shadow-none  rouned-lg hover:opacity-75 transition">
    ...
    <div className="flexx flex-col  gap-y-4 col-span-1">
      <div className="bg-white  border rounded-none p-4">
        ...
        <ul className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {data.map((member) => (
            <li key={member.$id}>
              <Card className="shadow-none  rouned-lg  overflow-hidden">
